...
Last updated: 26 Sep 2026 (Big Top added, family features + Auto-resolution fix). Read this whole file before touching code. Where this file and the code disagree, **the code wins**. Read `public/games/dirt.js` and `public/shared/core.js` before writing a new game.
...
3. `public/sw.js`: add both files to `CORE`, bump `VERSION` (next: `xretro-v12`)
...
Status (checked 26 Sep): **Big Top** ✅ has all three (NEW BEST on act times and top score, medals per act on its act board and index card, awards per act and for the whole run). Awards also exist in **Blacktop Brawl** and **Strike Force** (their own code, not the shared helper yet). **Still missing:**
...
| 5 | **Big Top** | Circus Charlie | ✅ Built and tested 26 Sep; sent as `big-top.zip`, waiting for the owner's push. 1–4 co-op circus platformer, shared camera, 6 acts + 3 bosses, hoops, trampolines, cannons, trapezes, tightropes, balloons, hidden bells + bonus tent, medals/awards/NEW BEST, Kids/Normal/Pro, drop-in, online |
